...
export class MantenedorComponent  implements OnInit {
  ...
  loadSandwichData(id: number): void {
    this.sandwichService.getSandwichById(id).subscribe(sandwich => {
      if (sandwich) {
  ...
  onSubmit(): void {
    if (this.sandwichForm.valid) {
      const sandwichData: Sandwich = this.sandwichForm.value;
      if (this.isEditMode && this.sandwichId) {
        sandwichData.id = this.sandwichId; 
        this.sandwichService.updateSandwich(sandwichData).subscribe(
          updatedSandwich => {
            if (updatedSandwich) {
        ...
        this.sandwichService.addSandwich(sandwichData).subscribe(
          newSandwich => {
            if (newSandwich) {
